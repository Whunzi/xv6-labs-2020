...
//wqj 释放mmap映射的页 根据PTE_D和MAP_SHARED判断是否将修改写回磁盘

void vmaunmap(pagetable_t pagetable, uint64 va,uint64 nbytes,struct wqj_vma* v){
  uint64 a;
  pte_t* pte;

  for(a = va; a < va + nbytes; a = a + PGSIZE){
    if((pte = walk(pagetable,a,0)) == 0){
      continue;
    }
